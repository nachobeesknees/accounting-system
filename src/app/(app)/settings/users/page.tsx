...
export default async function UsersPage({
  ...
  // Spec: only super_admin and admin can access this page.
  if (me.role !== "super_admin" && me.role !== "admin" && !me.isSuperuser) {
    redirect("/settings");
  }
  ...
  const canUpdate = hasPermission(me, "user.update");
  ...
  return (
    <>
      ...
      <div className="px-6 py-3.5 flex flex-col gap-3.5">
        ...
        {accessUserId && canAssign && (
          <Card
            title={`Entity access — ${
              users.find((u) => u.id === accessUserId)?.email ?? accessUserId
            }`}
          >
            <form
              action={setEntityAccessAction}
            ...
            >
              ...
              <div className="text-[12px]" style={{ color: "var(--ink-3)" }}>
                ...
                entities selected, the user sees all entities (the admin
                default). Use read-only to allow viewing without write access.
                Note: super_admin and admin roles ignore this restriction.
              </div>
              <div
                style={{
                  display: "grid",
        ...
        <Card title="Role legend">
          ...
          >
            {ALL_ROLES.map((r: Role) => (
              <div key={r} style={{ color: "var(--ink-2)" }}>
                ...
                {r === "employee" && "Read-only on assigned clients only."}
              </div>
            ))}
          </div>
        </Card>
